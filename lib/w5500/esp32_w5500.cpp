                        ...
                        uint8_t *W5500_Mac)
{
  ...
  if ( (SPICLOCK_MHZ < 14) || (SPICLOCK_MHZ > 25) )//asalnya 14 min
  {
    Serial.println("SPI Clock must be >= 8 and <= 25 MHz for W5500");
...
bool ESP32_W5500::config(IPAddress local_ip, IPAddress gateway, IPAddress subnet, IPAddress dns1, IPAddress dns2)
{
  ...
  {
    printf("DHCP could not be stopped! Error =", err);
    return false;
  }
  ...
  if (err != ERR_OK)
  {
    printf("STA IP could not be configured! Error = ", err);
    return false;
  }
  ...
  else
  {
    ...
    {
      printf("DHCP could not be started! Error =", err);
      return false;
    }
...
IPAddress ESP32_W5500::localIP()
{
  ...
  printf("localIP =", IPAddress(ip.ip.addr));
